...
/*************************************************************************************************/
  ...
  /**************************************************************************************/
    ...
    /***************************    independent sensors  独立传感器  ********************************/
      ...
      //#define NUNCHUCK  // if you want to use the nunckuk connected to a WMP（Wii Motion Plug Wii动作插件）
      //#define MMA7455
      //#define ADXL345
      //#define BMA020
      //#define BMA180
      ...
      /* I2C magnetometer 磁强计*/
      //#define HMC5843
      //#define HMC5883
      //#define AK8975
      //#define MAG3110

      /* Sonar 声呐*/ // for visualization purpose currently - no control code behind
      //#define SRF02 // use the Devantech SRF i2c sensors
      //#define SRF08
      //#define SRF10
      //#define SRF23

      /* ADC accelerometer 模数转换加速度计 */ // for 5DOF from sparkfun, uses analog PIN A1/A2/A3
      //#define ADCACC
...
/*************************************************************************************************/
/*****************                                                                 ***************/
/****************  SECTION  2 - COPTER TYPE SPECIFIC OPTIONS           旋翼机类型细节设置                    *******/
/*****************                                                                 ***************/
/*************************************************************************************************/

  /********************************    TRI    *********************************/
    #define YAW_DIRECTION 1
    //#define YAW_DIRECTION -1 // if you want to reverse the yaw correction direction 方向角设置
    /* you can change the tricopter servo travel here 三轴舵机行程*/
      #define TRI_YAW_CONSTRAINT_MIN 1020
      #define TRI_YAW_CONSTRAINT_MAX 2000
      #define TRI_YAW_MIDDLE 1500 // (*) tail servo center pos. - use this for initial trim; later trim midpoint via LCD

  /********************************    BI    *********************************/
    /* you can change the bicopter servo travel direction here 两轴舵机行程方向*/     
    //#define BI_PITCH_DIRECTION 1
     #define BI_PITCH_DIRECTION -1

   /********************************    ARM/DISARM    *********************************/
   /* optionally disable stick combinations to arm/disarm the motors.选项禁止组合摇杆解锁/上锁电机
     * In most cases one of the two options to arm/disarm via TX stick is sufficient 多数情况下选择一个TX摇杆解锁足够*/
    #define ALLOW_ARM_DISARM_VIA_TX_YAW
    //#define ALLOW_ARM_DISARM_VIA_TX_ROLL

  /***********************          Cam Stabilisation   相机云台自稳定          ***********************/
  ...
  /***********************          Flying Wing    飞行翼               ***********************/
    /* you can change servo orientation and servo min/max values here 可改变舵机方向和最小、最大值
  ...
  /***********************      Common for Heli & Airplane   直升机和固定翼通用设置      ***********************/
  ...
  /***********************      Single and Dual Copter Settings  单轴和双轴旋翼机设置   ***********************/
    /* Change to -1 to reverse servo movement per axis每个轴向的舵机动作
...
/*************************************************************************************************/
  ...
  /* note: no need to uncomment something in this section if you use a standard receiver 如果使用标准接收机，无需非注释某些设置 */

  /**************************************************************************************/
  /********                       special receiver types    特别接收机类型         ********************/
  /**************************************************************************************/

    /****************************    PPM Sum Reciver    ***********************************/
      /* The following lines apply only for specific receiver with only one PPM sum signal, on digital PIN 2
         Select the right line depending on your radio brand. Feel free to modify the order in your PPM order is different */
      //#define SERIAL_SUM_PPM         PITCH,YAW,THROTTLE,ROLL,AUX1,AUX2,AUX3,AUX4,8,9,10,11 //For Graupner/Spektrum不同接收机设置
    ...
    /**********************    Spektrum Satellite Reciver Spektrum卫星接收机   *******************************/
    ...
    /*******************************    SBUS RECIVER FutabaSBUS协议接收机   ************************************/
      /* The following line apply only for Futaba S-Bus Receiver on MEGA boards at RX1 only (Serial 1).
         You have to invert the S-Bus-Serial Signal e.g. with a Hex-Inverter like IC SN74 LS 04 */
      //#define SBUS

    /******************* RC signal from the serial port via Multiwii Serial Protocol 使用MultiWii串口协议获得遥控信号*********/
      //#define RCSERIAL
...
/*************************************************************************************************/
/*****************                                                                 ***************/
/****************  SECTION  4 - ALTERNATE CPUs & BOARDS   备份CPUs和主控板                                 *******/
/*****************                                                                 ***************/
/*************************************************************************************************/

  /**************************************************************************************/
  /********                      Promini Specifig Settings    ArduinoPromini的设置       ********************/
  /**************************************************************************************/

    /**************************    Hexa Motor 5 & 6 Pins 6轴5，6脚设置   *******************************/
  ...
  /**************************************************************************************/
  /********   Settings for ProMicro, Leonardo and other Atmega32u4 Boards  32u4板设置   ***********/
  /**************************************************************************************/
  ...
  /**************************************************************************************/
  /********                      override default pin assignments  扩展默认引脚分配  ********************/
  /**************************************************************************************/
...
/*************************************************************************************************/
/*****************                                                                 ***************/
/****************  SECTION  5 - ALTERNATE SETUP        备份设置                                    *******/
/*****************                                                                 ***************/
/*************************************************************************************************/
  ...
  /**************************************************************************************/
    ...
    /*********************    Lowpass filter for some gyros  某些陀螺仪的低通滤波  ****************************/
      ...
      /* MPU6050 Low pass filter setting. In case you cannot eliminate all vibrations to the Gyro, you can try
         ...
         balancing options ran out. Uncomment only one option!
         IMPORTANT! Change low pass filter setting changes PID behaviour, so retune your PID's after changing LPF.
		 你无法消除所有的陀螺仪震动，可尝试减小低通滤波频率，只有一步一步尝试。当振颤消除时，使用这个设置。
		 这不会帮助振动的反馈，所以只是飞行器随机震颤和阻尼、平衡设置全部失效。
		 重要！改低通滤波设置会改变PID行为，所以改变LPF后要重新设置PID*/
      //#define MPU6050_LPF_256HZ     // This is the default setting, no need to uncomment, just for reference 默认设置，仅作参考，可不解除注释
...
/*************************************************************************************************/
  ...
  /************************        continuous gyro calibration   连续陀螺仪校准     ********************/
  ...
  /*************************    INFLIGHT ACC Calibration  飞行中加速度计校准  *****************************/
    /* This will activate the ACC-Inflight calibration if unchecked */
    //#define INFLIGHT_ACC_CALIBRATION

  /**************************    Disable WMP power pin  禁用WMP电源接口   *******************************/
  ...
  /**************************************************************************************/
    ...
    /* introduce a deadband around the stick center 在摇杆中心周边简历死区
...
/*************************************************************************************************/
/*****************                                                                 ***************/
/****************  SECTION  7 - TUNING & DEVELOPER        调教和开发                          **************/
/*****************                                                                 ***************/
/*************************************************************************************************/
